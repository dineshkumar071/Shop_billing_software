...
class product:
	def __init__(self,name,quantity,weight,rate,tax,discount):
		self.name=name
		self.quantity=quantity
		self.weight=weight
		self.rate=rate
		self.tax=tax
		self.total=(weight*rate)-discount
...
def printinvoice():
    ...
    pdf= canvas.Canvas("G:\\bill\\"+str(int(head.InvoiceNumber))+".pdf")
    ...
    for item in Products:
        currproduct=product(item[0],item[1],item[2],item[3],item[6],item[4])
    ...
    pdf.save()
    webbrowser.open("G:\\bill\\"+str(int(head.InvoiceNumber))+".pdf")
...
def SubmitData():
     ...
     dis=float((weight*rate)*(DISCOUNT.get()/100))
     total=float(WEIGHT.get()*RATE.get()-dis)
     tax=float(TAX.get()*0.01*total)

     Products.append((product,quantity,weight,rate,dis,total,tax))
...
def AddNewWindow():
    ...
    lbl_weight = Label(Form, text="Weight", font=('arial', 14), bd=5)
    lbl_weight.grid(row=2, sticky=W)
    lbl_rate = Label(Form, text="Rate", font=('arial', 14), bd=5)
    lbl_rate.grid(row=3, sticky=W)
    taxlabel=Label(Form,text="Tax(%)",font=('arial',14),bd=5)
    taxlabel.grid(row=4,sticky=W)
    discountLabel= Label(Form,text="Discount(%)",font=('arial',14),bd=5)
    discountLabel.grid(row=5,sticky=W)
    ...
    weight = Entry(Form, textvariable=WEIGHT, font=('arial', 14))
...
#============================LABELS======================================
lbl_title = Label(Top, text="THANGAM JEWELLERS", font=('arial', 16), width=500)
lbl_title.pack(fill=X)
var = StringVar()
today =date.today()
today=today.strftime("%d/%m/%Y")
var.set("Date: " + today)
label = Label(Top,font=('arial', 12), width=500,textvariable=var, relief=RAISED )
label.pack(fill=X)
...
tree = ttk.Treeview(TableMargin, columns=("Product Name","Quantity","Weight", "Rate","Discount", "Total", "Tax"), height=400, selectmode="extended", yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
scrollbary.config(command=tree.yview)
scrollbary.pack(side=RIGHT, fill=Y)
scrollbarx.config(command=tree.xview)
scrollbarx.pack(side=BOTTOM, fill=X)
tree.heading('Product Name', text="Product Name", anchor=W)
tree.heading('Quantity', text="Qunatity", anchor=W)
